  ...
  jobid="structure_027173108"
  outpath="/mnt/efs/iter-3/structure_027173108/mlrunner"
  ...
  cd $outpath
  cmd="mummi-ml start --jobid ${jobid} --workspace=${ws} --outdir=${outpath} --tag mlrunner --encoder-model ${model} --ml-outdir=${ws} --feedback --resources ${resources} --complex=${complex}"
  NUM_THREADS=$(nproc)
