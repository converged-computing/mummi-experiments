...
flux module unload sched-simple || true
flux module load /usr/lib/flux/modules/sched-fluxion-resource.so  || true
flux module load /usr/lib/flux/modules/sched-fluxion-qmanager.so  || true

# In practice the mount won't work - I think terraform creates the filesystem
# after the instances. So we use flux to mount across.
